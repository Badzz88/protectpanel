...
class NodesController extends Controller
{
    ...
    public function store(NodeFormRequest $request): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "🚫 Kamu tidak punya izin untuk menambahkan node. Hanya admin ID 1 yang bisa!. © ʙʏ ʙᴀᴅᴢᴢ");
        }
    ...
    public function updateSettings(NodeFormRequest $request, Node $node): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "⚠️ AKSES DI TOLAK. HANYA ADMIN ID 1 YANG BISA EDIT NODE. © ʙʏ ʙᴀᴅᴢᴢ");
        }
    ...
    public function delete(int|Node $node): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "❌  𝐋𝐔 𝐒𝐈𝐀𝐏𝐀 𝐓𝐎𝐋𝐎𝐋 𝐌𝐀𝐔 𝐇𝐀𝐏𝐔𝐒 𝐍𝐎𝐃𝐄 𝐆𝐖?. © ʙʏ ʙᴀᴅᴢᴢ");
        }
...
echo "🔒 Hanya Admin ID 1 yang bisa mengelola Nodes. © ʙʏ ʙᴀᴅᴢᴢ"