    ...
    public function handle(Server $server): void
    {
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('❌Akses ditolak: Lu cuman bisa hapus server sendiri tolol. © ʙʏ ʙᴀᴅᴢᴢ');
                }
            }
        }
...
echo "⚠️ Hanya Admin ID 1 yang bisa hapus server lain. © ʙʏ ʙᴀᴅᴢᴢ"